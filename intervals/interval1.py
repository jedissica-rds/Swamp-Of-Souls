...
import constants
from levels import player as player_mod
...
game_screen = pygame.display.set_mode((constants.WIDTH, constants.HEIGHT))  # Changed 'screen' to 'game_screen'
...
jar_positions = [constants.WIDTH - 60, constants.HEIGHT - 150]
...
class Firefly(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.x < 0 or self.rect.x > constants.WIDTH - self.rect.width:
            self.vx = -self.vx
        if self.rect.y < 0 or self.rect.y > constants.HEIGHT - self.rect.height:
            self.vy = -self.vy
...
class IntervalScreen:
    ...
    def run(self):
        ...
        while running:
            game_screen.fill(constants.WHITE)
            clock.tick(FPS)
            ...
            if abs((self.player_position[0] + 36) - jar_positions[0]) < 70:
                press_e_text = constants.x_small_font.render(f'Pressione E para pegar a jarra', True, constants.WHITE)
                game_screen.blit(press_e_text, (jar_positions[0] + 5, jar_positions[1] - 60))

            # Draw the tip
            text = constants.small_font.render(f'Aqui é muito escuro...Como posso iluminar meu caminho?', True, constants.WHITE)
            game_screen.blit(text, (50, constants.HEIGHT - 60))
